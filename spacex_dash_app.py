...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        fig = px.pie(spacex_df[['Launch Site', 'class']], values='class', 
        names='Launch Site', 
        title='Total Success Launches by Site')
        return fig
    else:
        # return the outcomes piechart for a selected site
        filtered_data = spacex_df[spacex_df['Launch Site'] == entered_site].groupby('class').size().reset_index(name='class count')
        fig = px.pie(filtered_data, values='class count', 
        names='class', 
...
def get_scatter_chart(entered_site, payload):
    if entered_site == 'ALL':
        ...
        fig= px.scatter(new_df3, x='Payload Mass (kg)', y='class',
